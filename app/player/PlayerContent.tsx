...
const PlayerContent = () => {
  ...
  const [audio, { playing, buffered, time, duration }, controls, ref] = useAudio({
    src: activeSong?.src || '',
    autoPlay: true,
  });
  ...
  return (
    <>
      <Slider className="absolute left-0 right-0 py-1" max={duration} defaultValue={[0]} value={[time]} onValueChange={value => controls.seek(value[0])} />
      {audio}
      <div className="flex items-center justify-center mt-6">
        ...
        <div className="flex justify-center gap-2 min-w-28 text-sm text-neutral-500"><span>{second}:{milliSecond}</span> / {playTime[0]}:{playTime[1]}</div>
      </div>
    </>
  )
}

...
const PlayListCard = ({ playlist }: { playlist: Playlist }) => {
  ...
  const [randomImage, setRandomImage] = useState<string>(songList[0].imageSrc);
  ...
  useEffect(() => {
    if (songList?.length) {
      setRandomImage(getRandomElementFromArray(songList).imageSrc);
    }
  }, [songList]);
  ...
  return (
    <dl className="flex flex-col relative cursor-pointer group h-full" onClick={() => push(`/playlist?list=${id}`)}>
      ...
      <dd className="relative h-[136px] -order-last"><Image src={randomImage} fill sizes="100%" className="object-cover rounded-sm" alt="thumbnail" /></dd>
